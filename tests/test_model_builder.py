...
def test_lower_limb_functional_origins_use_expected_virtual_points():
    ...
    assert segments["Trunk"].frame.origin.parent_marker_names == (
        "LPSI",
        "RPSI",
        "LASI",
        "RASI",
    )
    assert segments["Trunk"].frame.origin.child_marker_names == (
        "T10",
        "T6",
        "C7",
        "C2",
...
def test_functional_axis_projection_reprojects_static_point_onto_transferred_axis(
    ...
    np.testing.assert_allclose(projected_point(static_data, BiomechanicalModelReal()), [0.5, 0.0, 0.0, 1.0])
...
def test_functional_sara_axis_falls_back_when_static_axis_deviation_is_too_large(
    ...
    def marker(position):
        return np.asarray([position[0], position[1], position[2], 1.0], dtype=float).reshape(4, 1)
    ...
    monkeypatch.setattr(SegmentCoordinateSystemUtils, "sara", lambda **_kwargs: FakeAxis())
    ...
    np.testing.assert_allclose(sara_axis.start.function(data, model).reshape(4), [0.0, 0.0, 0.0, 1.0])
    np.testing.assert_allclose(sara_axis.end.function(data, model).reshape(4), [1.0, 0.0, 0.0, 1.0])
